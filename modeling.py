...
from transformers import CLIPProcessor, CLIPModel, LlamaTokenizer
from torch import Tensor
...
from transformers import CLIPConfig, WhisperConfig, LlamaConfig, AutoConfig
from transformers.modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast, SequenceClassifierOutputWithPast
from transformers import LlamaConfig, WhisperConfig, WhisperPreTrainedModel, WhisperModel, AutoModel, ClapAudioModel, ClapProcessor, ClapAudioConfig
...
IS_CROSS_ATTN = Constants.IS_CROSS_ATTN
IS_INTERLEAVED_PROMPTING = Constants.IS_INTERLEAVED_PROMPTING
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    cos = cos[position_ids].unsqueeze(1).to(q.device)  # [bs, 1, seq_len, dim]
    sin = sin[position_ids].unsqueeze(1).to(q.device)  # [bs, 1, seq_len, dim]
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids.cpu())
        # [bsz, nh, t, hd]
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def OT_dist(self, v_, q_, got_lambda_wd=0.1, got_lambda_gwd = 0.1):
        ...
        gwd = GW_distance_uniform(v_.transpose(2,1), q_.transpose(2,1), device=v_.device)
        twd = got_lambda_wd * torch.mean(wd) #  + got_lambda_gwd * torch.mean(gwd)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            print('Shapes before view ======>', shift_logits.shape, shift_labels.shape)
            ...
            amax = torch.argmax(shift_logits, dim=1)
            
            index_518 = torch.where(shift_labels == 518)[0] if len(torch.where(shift_labels == 518)[0]) == 1 else torch.where(shift_labels == 518)[0][-1]
        
            tmp_shift_labels = torch.ones_like(shift_labels) * -100
            tmp_shift_labels[index_518:] = shift_labels[index_518:]
            
            # Enable model parallelism
            tmp_shift_labels = tmp_shift_labels.to(shift_logits.device)
            loss = loss_fct(shift_logits, tmp_shift_labels)
            
            print('========> Modelling loss: ', loss)
            
            epsilon = 0.1
            if mask_labels is not None and vid_aud_cross is not None:
                mask_labels = mask_labels.contiguous()
                mask_labels = mask_labels.to(shift_logits.device)
                loss_align = 0.5 * (mask_labels * vid_aud_cross).sum()/(mask_labels.sum()+epsilon) + 0.5 * (1 - ((1 - mask_labels) * vid_aud_cross).sum()/((1 - mask_labels).sum()+epsilon))  # 0. * shift_logits.mean() +
                
                if Constants.IS_OT:
                    total_loss = loss + Constants.OT_WEIGHT * self.OT_dist(vid_loc_feats, aud_loc_feats)
                else:
                    total_loss = loss
                    
                if IS_CROSS_ATTN:
                    total_loss = loss + Constants.CROSS_ATTN_WEIGHT * loss_align  
                else:
                    total_loss = loss
            else:
                total_loss = loss     
...
class MM_LLMs_Config(PretrainedConfig):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        whisper_config = ClapAudioConfig.from_dict(config_dict['audio_config'])
...
class MM_LLMs(PreTrainedModel):
    def __init__(self, config):
        ...
        self.audio_encoder = ClapAudioModel.from_pretrained("laion/clap-htsat-fused")
        self.audio_processor = ClapProcessor.from_pretrained("laion/clap-htsat-fused")

        self.llm = LlamaForCausalLM(config.llm_config)
        self.llm.from_pretrained('meta-llama/Llama-2-7b-chat-hf')

        if Constants.IS_LORA_ENABLED:
            print('\n\n\n============= LORA IS ENABLED!!! ===========\n\n\n')
            from peft import LoraConfig, get_peft_model, TaskType

            lora_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM,
                inference_mode=Constants.IS_INFERENCE_MODE,
                r=32,
                lora_alpha=32,
                lora_dropout=0.1,
        ...
        self.video_audio_cross_attention = nn.MultiheadAttention(config.image_config.projection_dim,
                                                            config.attention_heads,
                                                            dropout=attn_dropout,
                                                            add_bias_kv=is_add_bias_kv,
                                                            add_zero_attn=is_add_zero_attn,
    ...
    def forward(self, inputs=None, return_alignment_loss=False):
        # """
        # :param inputs:
        #             video_frames: (B x F)
        #             audios: B x 1
        ...
        # """
        text_embeddings, attention_mask, labels, mask_labels, vid_aud_cross, vid_loc_feats, aud_loc_feats = self.prepare_inputs_for_generation(inputs)
        ...
        outputs = self.llm(inputs_embeds=text_embeddings, attention_mask=attention_mask, labels=labels, mask_labels = mask_labels if return_alignment_loss else None, vid_aud_cross=vid_aud_cross, vid_loc_feats = vid_loc_feats, aud_loc_feats = aud_loc_feats)
    ...
    def prepare_inputs_for_generation(self, inputs):
        ...
        if Constants.IS_LORA_ENABLED:
            embed_tokens = self.llm.get_base_model().model.embed_tokens
        else:
        ...
        dummy_tokenizer = LlamaTokenizer.from_pretrained('trained_models/meta_llama_7B')
        tokenized_ops = dummy_tokenizer.batch_decode(inputs['input_ids'], skip_special_tokens=True, clean_up_tokenization_spaces=False)

        pattern = r'\d+\.\d+|\d+' # r'\d+'
        bb_list = []
        video_audio_cross_attn = None

        if Constants.IS_SIMPLE_PROMPT:
            simple_weight = 224/100
        else:
            simple_weight = 224

        for tokenized_op in tokenized_ops:
            bb_XYXY_224 = re.findall(pattern, tokenized_op.split("Response:")[-1])
            if len(bb_XYXY_224) == 4:
                bb_XYXY_224 = [np.float32(x)*simple_weight for x in bb_XYXY_224]
                bb_list.append(bb_XYXY_224)
            else:
                bb_list.append([0.,0.,0.,0.])
                
        ### BELOW PART IS FOR CROSS ATTENTION  #######
        mask_labels = None
        if audio_features is not None and video_features is not None:
            vid_loc_feats, aud_loc_feats = None, None
            if Constants.IS_OT:
                vid_loc_feats, aud_loc_feats = torch.nn.AdaptiveAvgPool1d(512)(video_features), torch.nn.AdaptiveAvgPool1d(512)(audio_features)

            if IS_CROSS_ATTN:
                video_features, video_audio_cross_attn = self.video_audio_cross_attention(video_features, torch.nn.AdaptiveAvgPool1d(512)(audio_features), torch.nn.AdaptiveAvgPool1d(512)(audio_features))
            else:
                video_features_cross, video_audio_cross_attn = self.video_audio_cross_attention(video_features, torch.nn.AdaptiveAvgPool1d(512)(audio_features), torch.nn.AdaptiveAvgPool1d(512)(audio_features))

            video_audio_cross_attn = video_audio_cross_attn.mean(dim=-1)
            b_s = video_audio_cross_attn.shape[0]
            video_audio_cross_attn = video_audio_cross_attn.reshape(b_s, 196, video_audio_cross_attn.shape[1] // 196)
            video_audio_cross_attn = video_audio_cross_attn.mean(dim=-1)
            video_audio_cross_attn = video_audio_cross_attn.reshape(b_s, 14, 14).unsqueeze(dim=1)
            video_audio_cross_attn = torch.nn.functional.interpolate(video_audio_cross_attn, size=(224,224), mode='bilinear', align_corners = True).squeeze(dim=1)
            ...
            for bb_idx in range(len(bb_list)):
                ...
                mask_1 = torch.ones(224, 224)
                if np.sum(bb) > 0:
                    mask_1[bb[1]:bb[3], bb[0]:bb[2]] *= 0.
                mask_labels.append(mask_1)
        ...
        if video_features is not None:
            ...
            try:
                ...
                if not IS_INTERLEAVED_PROMPTING:
                    text_embeddings = torch.cat([torch.cat([text_embeddings[:, 0, :].unsqueeze(1), video_inputs], dim=1), text_embeddings[:, 1:, :]], dim=1)
                else:
                    mixed_embeds = []
                    for idx,tokenized_op in enumerate(tokenized_ops):
                        st_lst = tokenized_op.split('video', 1)
                        st_lst[0] = st_lst[0] + 'video'
                        seg_tokens = [dummy_tokenizer(st_lst[0],return_tensors='pt').to(text_embeddings.device).input_ids]
                        max_len = 256 - seg_tokens[0].shape[1] + 1
                        seg_tokens += [dummy_tokenizer(st_lst[1],max_length=max_len, padding='max_length', truncation=True,return_tensors='pt').to(text_embeddings.device).input_ids[0][1:]]
                        seg_embs = [embed_tokens(seg_tokens[0])[0], embed_tokens(seg_tokens[1])]
                        mixed_embs = [seg_embs[0],video_inputs[idx,:],seg_embs[1]]

                        mixed_embs = torch.cat(mixed_embs, dim=0).unsqueeze(0)
                        mixed_embeds.append(mixed_embs)

                    text_embeddings = torch.cat(mixed_embeds, dim=0)

                ingore_num += (video_inputs.size(1))
            except:
                pass

        if audio_features is not None and Constants.IS_AUDIO_MODALITY:
        ...
        return text_embeddings, attention_mask, labels, mask_labels, video_audio_cross_attn, vid_loc_feats, aud_loc_feats  #loss_align  #loss_align
    ...
    def encode_video_long(self, videos):
        ...
        self.video_encoder.vision_model = self.video_encoder.vision_model.to(self.video_encoder.device) 
    ...
    def encode_audio(self, audios):
        is_longer = torch.ones((audios.shape[0], 1), dtype=torch.bool)
        is_longer = is_longer.to(audios.device)
        try:
            audio_features = self.audio_encoder(input_features=audios, is_longer=is_longer)
            audio_features = torch.flatten(audio_features.last_hidden_state,2)
    ...
    def encode_image(self, images):
        ...
        self.image_encoder.vision_model = self.image_encoder.vision_model.to(self.image_encoder.device) 
